...
cv::Mat frequency_filter(cv::Mat &padded,cv::Mat &blur)
{
    ...
    cv::idft(complexIm, complexIm, 1);   //idft
    cv::Mat dst_plane[2];
...
//                std::cout <<  "notch_pot: " << notch_pot.at(k) << std::endl;
    ...
    cv::Mat show;
    cv::normalize(temp[0], show, 1, 0, cv::NORM_MINMAX);
    cv::imshow(name, show);
    return notch_pass;
}
...
int main(int argc, char * argv[])
{
    ...
    if( !notch_point.empty() ){
        for( unsigned int i = 0; i < notch_point.size(); i++ ){
            ...
            std::cout << "notch point location: " << notch_point.at(i) << std::endl;
        }
    }
    ...
    cv::normalize(dst_plane[0], dst_plane[0], 1, 0, cv::NORM_MINMAX);
