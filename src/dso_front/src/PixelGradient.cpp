...
void PixelGradient::computeGradentsFor16(const Mat img)
{
  // 初始的长宽
  int w = img.cols;
  int h = img.rows;
  ...
  // 将原来在img中的数据拷贝到color分配的内存中
  // 对于16位这一步有问题
  // memcpy(color, (unsigned short*)img.data, img.rows*img.cols);
  for (int j = 0; j < img.rows; ++j) {
    for (int i = 0; i < img.cols; ++i) {
      color[i + j * img.cols] = (img.at<ushort>(j, i));
    }
  }
  ...
  for (int i = 0; i < w * h; i++){
    dI[i][0] = (float)color[i];
      // TODO:
    if((float)color[i] > vmax)
      vmax = (float)color[i];
    if((float)color[i] < vmin)
      vmin = (float)color[i];
  }
  ...
  cv::Mat out(img.size(), CV_16UC1);
  toCvMat16(absSquaredGrad[0], out);
  cv::Mat out2(out.size(), CV_8UC1);
  for (int j = 0; j < out2.rows; ++j) {
    for (int i = 0; i < out2.cols; ++i) {
      (out2.at<uchar>(j, i)) = (uchar)((float)(out.at<ushort>(j,i)/(vmax - vmin))*255.0); 
    }
  }
  cv::imshow("hi", out2);
  cv::waitKey();
